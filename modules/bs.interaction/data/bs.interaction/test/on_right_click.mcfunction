...
execute store success score #s bs.ctx run function #bs.interaction:on_right_click { run: "", executor: "source" }
...
execute as @n[type=minecraft:interaction,tag=bs.packtest,distance=..2] store success score #s bs.ctx run function #bs.interaction:on_right_click { run: "", executor: "target" }
execute unless score #s bs.ctx matches 0 run fail "Failed to return an error when the command is invalid"
execute as @n[type=minecraft:interaction,tag=bs.packtest,distance=..2] store success score #s bs.ctx run function #bs.interaction:on_right_click { run: "help", executor: { selector: "¯\\_(ツ)_/¯" } }
execute unless score #s bs.ctx matches 0 run fail "Failed to return an error when the selector is invalid"
execute as @n[type=minecraft:interaction,tag=bs.packtest,distance=..2] store success score #s bs.ctx run function #bs.interaction:on_right_click { run: "help", executor: { selector: "@n[type=minecraft:allay,tag=bs.is_never_gonna_be_released]", lazy: false } }
