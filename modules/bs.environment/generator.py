...
from bookshelf.minecraft import Biome, get_biomes
from bookshelf.utils import gen_loot_table_tree, render_snbt
...
def beet_default(ctx: Context) -> None:
    ...
    biomes = get_biomes(ctx)

    with ctx.override(generate_namespace=namespace):
        ctx.generate("can_snow", render=gen_can_snow(biomes))
        ctx.generate("has_precipitation", render=gen_has_precipitation(biomes))
        ctx.generate("get/get_biome", render=gen_get_biome(biomes))


def gen_get_biome(biomes: list[Biome]) -> LootTable:
...
def gen_can_snow(biomes: list[Biome]) -> Predicate:
...
def gen_has_precipitation(biomes: list[Biome]) -> Predicate:
